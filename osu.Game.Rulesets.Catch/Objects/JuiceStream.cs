...
{
    public class JuiceStream : CatchHitObject, IHasPathWithRepeats, IHasSliderVelocity, IHasGenerateTicks
    {
        /// <summary>
        ...
        /// </summary>
        public bool UsesLegacyLastTick { get; set; }

        /// <summary>
        /// Whether LegacyLastTick prevented every tiny droplet to generate on the current path. 
        /// </summary>
        public bool IsLegacyLastTickMissAllTiny { get; set; }

        /// <summary>
        /// Whether LegacyLastTick prevented extra tiny droplets to generate on the current path. 
        /// </summary>
        public bool IsLegacyLastTickMissExtraTiny { get; set; }

        /// <summary>
        /// The amount of tiny droplets generated by LegacyLastTick on the current path. 
        /// </summary>
        public int LegacyLastTickTinyDropletCounter { get; set; }

        /// <summary>
        /// Whether the newly generated tiny droplet should use a new random generator. 
        /// </summary>
        ...
        protected override void CreateNestedHitObjects(CancellationToken cancellationToken)
        {
            ...
            foreach (var e in SliderEventGenerator.Generate(StartTime, SpanDuration, Velocity, TickDistance, Path.Distance, this.SpanCount(), cancellationToken))
            {
                ...
                if (lastEvent != null)
                {
                    ...
                    if (!UsesLegacyLastTick && e.Type is SliderEventType.LegacyLastTick)
                    {
                        ...
                        if (sinceLastTickWithLegacy <= 100)
                            IsLegacyLastTickMissAllTiny = true;
                        ...
                        else
                        {
                            ...
                            int tinyTickCountRealEndTime;
                            double sinceLastTickWithRealEndTime = (int)this.GetEndTime() - (int)lastEvent.Value.Time;
                            double legacyLastTickSlice = sinceLastTickWithLegacy;
                            double realEndTimeSlice = sinceLastTickWithRealEndTime;
                            ...
                            while (realEndTimeSlice > 100)
                            {
                                realEndTimeSlice /= 2;
                            }

                            tinyTickCountLegacyLastTick = (int)((sinceLastTickWithLegacy - legacyLastTickSlice) / legacyLastTickSlice);
                            tinyTickCountRealEndTime = (int)((sinceLastTickWithRealEndTime - realEndTimeSlice) / realEndTimeSlice);

                            //Check if there are missing extra tiny droplets compared to osu!stable
                            if (tinyTickCountRealEndTime > tinyTickCountLegacyLastTick)
                            {
                                IsLegacyLastTickMissExtraTiny = true;
                                LegacyLastTickTinyDropletCounter = tinyTickCountLegacyLastTick;
                            }
                        }
                    ...
                    if (sinceLastTick > 80)
                    {
                        ...
                        if (IsLegacyLastTickMissAllTiny && timeBetweenTiny > 100)
                            UsesNewRandom = true;
                        ...
                        for (double t = timeBetweenTiny; t < sinceLastTick; t += timeBetweenTiny)
                        {
                            ...
                            if (IsLegacyLastTickMissExtraTiny)
                            {
                                if (LegacyLastTickTinyDropletCounter > 0)
                                    LegacyLastTickTinyDropletCounter--;
                                else if (LegacyLastTickTinyDropletCounter == 0)
                                    UsesNewRandom = true;
                            }
