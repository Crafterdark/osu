...
namespace osu.Game.Rulesets.Catch.Beatmaps
{
    public class CatchBeatmapProcessor : BeatmapProcessor
    {
        ...
        private static int calculateDirection(PalpableCatchHitObject current, PalpableCatchHitObject next) => next.EffectiveX > current.EffectiveX ? 1 : -1;
        ...
        private static void initialiseHyperDash(IBeatmap beatmap)
        {
            ...
            //Original: The current beatmap without modifications.
            //Modified: The current beatmap with applicable modifications.
            ...
            for (int i = 0; i < palpableObjects.Length - 1; i++)
            {
                ...
                int originalCurrentDirection = calculateDirection(currentObject, nextObject);

                if (catchBeatmap.OriginalHyperDashGeneration.Value)
                    originalLastExcess = processHyperDash(currentObject, nextObject, originalHalfCatcherWidth, originalCurrentDirection, originalLastDirection, originalLastExcess, true);

                int modifiedCurrentDirection = calculateDirection(currentObject, nextObject);

                if (catchBeatmap.ModifiedHyperDashGeneration.Value)
                    modifiedLastExcess = processHyperDash(currentObject, nextObject, modifiedHalfCatcherWidth, modifiedCurrentDirection, modifiedLastDirection, modifiedLastExcess, false);

                originalLastDirection = originalCurrentDirection;
                modifiedLastDirection = modifiedCurrentDirection;
            }
        }

        private static double processHyperDash(PalpableCatchHitObject currentObject, PalpableCatchHitObject nextObject, double halfCatcherWidth, int currentDirection, int lastDirection, double lastExcess, bool regularHyperDashRemoval)
        {
            ...
            double distanceToNext = Math.Abs(nextObject.EffectiveX - currentObject.EffectiveX) - (currentDirection == lastDirection ? lastExcess : halfCatcherWidth);
            float distanceToHyper = (float)(timeToNext * Catcher.BASE_DASH_SPEED - distanceToNext);

            if (distanceToHyper < 0)
            {
                //HyperDash object is generated
            ...
            else
            {
                ...
                {
                    //HyperDash object already exists and doesn't need to be generated
                    return halfCatcherWidth;
                }
                else
                {
